...
كل حدث WebSocket يجب أن يُرسل **ضمن نفس الـ Prisma transaction** التي تُنفّذ تغيير الحالة وتسجيل AuditLog. القاعدة:

- العميل لا يجب أن يرى حالة جديدة (`status`) في الـ API بدون الحدث المقابل في WebSocket — والعكس صحيح
- إذا فشل إرسال WebSocket أو تغيير الحالة، تُلغى العملية كاملة (transaction roll-back)
- هذا يضمن أن كل حالة تغيير مُسجّلة في AuditLog ومُعلنة عبر WebSocket بشكل ذري
